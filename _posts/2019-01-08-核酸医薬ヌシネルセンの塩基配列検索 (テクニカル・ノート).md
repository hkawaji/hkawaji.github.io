...
* Bowtie2 - 次世代シーケンサー、特に短鎖配列のゲノムにマッピングの時に使われることが多い。ギャップ有りアラインメントができる。
...
* VMATCH - 繰り返し配列をしっかりとハンドリングできる検索ツール。
...
| bamToBed
```
無理矢理やなぁ...という力技(brute force)ですが、この程度のins/del数であれば下手に凝ったツールやパラメータを探したりするよりもよっぽど直球かもしれません。ins/del無しのときは編集距離2以内の結果すべてを出力 (--all -v 2)、ins/delが1個のときは編集距離1以内(--all -v 1)、ins/delが2個のときは編集距離0の結果すべてを出力 (--all -v 0)しています。
...
| tr '=' 'M' \
| samtools view --threads 50 -h -T hg38.fa -  \
| samtools calmd --threads 50 - hg38.fa \
| awk 'BEGIN{OFS="\t"}{
  if (match($0,"^@")){
    print
  }else{
